...
export function applyFixes(
	...
	const fixesByFile = new Map<string, Array<{ sourceId: string; targetId: string; inverseType: string; finding: IntegrityFinding }>>();

	for (const finding of fixable) {
		if (finding.category !== "MissingInverse") continue;
	...
	for (const [targetFile, fixes] of fixesByFile) {
		try {
			...
			const rels = fm.relationships as Array<{ target: string; type: string }>;
			let modified = false;

			for (const fix of fixes) {
				// Check if the inverse already exists (idempotent)
				const exists = rels.some(
					(r) => r.target === fix.sourceId && r.type === fix.inverseType,
				);
 ...
 */
function parseMissingInverseFinding(
	finding: IntegrityFinding,
	ctx: CheckContext,
): { sourceId: string; targetId: string; inverseType: string } | null {
	const match = finding.message.match(
		/^(\S+) --(\S+)--> (\S+) but \S+ has no (\S+) edge back/,
	);
	...
	if (!ctx.inverseMap.has(inverseType)) return null;
