...
const PROJECT_JSON_KNOWN_FIELDS: Record<string, string> = {
	name: "string",
...
const PLUGIN_CONFIG_KNOWN_FIELDS: Record<string, string> = {
	installed: "boolean",
	enabled: "boolean",
	path: "string",
...
function validatePluginManifest(filePath: string): SchemaFinding[] {
	const findings: SchemaFinding[] = [];
	const relPath = relative(process.cwd(), filePath);
...
function discoverAndValidate(projectRoot: string): SchemaFinding[] {
	const findings: SchemaFinding[] = [];
...
export async function runValidateSchemaCommand(args: string[]): Promise<void> {
	...
	// Group by file
	const byFile = new Map<string, SchemaFinding[]>();
