...
function runCreateSubcommand(args: string[]): void {
	...
	const linkPath = rawLinkPath as string;
	const options: SymlinkOptions = { force: flags.force };
...
function runVerifySubcommand(args: string[]): void {
	...
	const linkPath = positional[0] as string;
	const result: SymlinkVerification = verifySymlink(linkPath);
...
function runRemoveSubcommand(args: string[]): void {
	const { positional } = parseArgs(args);
...
function runStatusSubcommand(args: string[]): void {
	const { positional } = parseArgs(args);
