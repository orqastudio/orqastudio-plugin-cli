 ...
 */
function detectIsDirectory(resolvedTarget: string): boolean {
	try {
...
export function createSymlink(target: string, linkPath: string): void {
	...
	if (process.platform === "win32") {
		...
		execSync(
			`powershell -Command "New-Item -ItemType SymbolicLink -Path '${safeLinkPath}' -Target '${safeAbsTarget}' -Force"`,
			{ stdio: "pipe" },
		);
	} else {
 ...
 */
export function verifySymlink(linkPath: string): SymlinkVerification {
