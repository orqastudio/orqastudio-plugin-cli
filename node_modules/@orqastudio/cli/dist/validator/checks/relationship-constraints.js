 ...
 */
export function checkRelationshipConstraints(graph, ctx) {
    const findings = [];
    ...
    for (const node of graph.nodes.values()) {
        for (const ref of node.referencesOut) {
            if (!ref.relationshipType)
                continue;
            const constraint = constraints.get(ref.relationshipType);
            if (!constraint)
                continue;
            // Source type check
            if (constraint.fromTypes.length > 0 &&
...
//# sourceMappingURL=relationship-constraints.js.map