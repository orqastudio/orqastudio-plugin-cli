 ...
 */
export function checkMissingInverses(graph, ctx) {
    const findings = [];
    ...
    for (const node of graph.nodes.values()) {
        for (const ref of node.referencesOut) {
            ...
            if (!expectedInverse)
                continue; // unknown vocab — caught by VocabularyCompliance
            const target = graph.nodes.get(ref.targetId);
            if (!target)
                continue; // broken ref — caught by BrokenLinks
            ...
            if (constraint) {
                if (constraint.from.length > 0 && !constraint.from.includes(node.artifactType))
                    continue;
                if (constraint.to.length > 0 && !constraint.to.includes(target.artifactType))
                    continue;
            }
            const hasInverse = target.referencesOut.some((r) => r.field === "relationships" &&
...
//# sourceMappingURL=missing-inverses.js.map