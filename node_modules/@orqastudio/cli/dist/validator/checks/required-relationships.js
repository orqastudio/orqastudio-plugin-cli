 ...
 */
export function checkRequiredRelationships(graph, ctx) {
    const findings = [];
    ...
    for (const rel of ctx.relationships) {
        ...
        const from = rel.from ?? [];
        if (from.length === 0)
            continue; // unconstrained source — skip
        requiredDefs.push({
            key: rel.key,
            fromTypes: new Set(from),
            minCount: typeof constraints["minCount"] === "number" ? constraints["minCount"] : 1,
        });
    }
    ...
    for (const node of graph.nodes.values()) {
        for (const def of requiredDefs) {
            if (!def.fromTypes.has(node.artifactType))
                continue;
            const count = node.referencesOut.filter((r) => r.relationshipType === def.key).length;
...
//# sourceMappingURL=required-relationships.js.map