...
export async function runGraphCommand(args) {
    ...
    if (statusIdx >= 0 && args[statusIdx + 1])
        options.status = args[statusIdx + 1];
    const relatedIdx = args.indexOf("--related-to");
    if (relatedIdx >= 0 && args[relatedIdx + 1])
        options.relatedTo = args[relatedIdx + 1];
    const relTypeIdx = args.indexOf("--rel-type");
    if (relTypeIdx >= 0 && args[relTypeIdx + 1])
        options.relationshipType = args[relTypeIdx + 1];
...
function printArtifactDetail(node, allNodes) {
    console.log(`\n${node.id}: ${node.title}`);
...
function printTree(results, allNodes) {
    ...
    for (const milestone of milestones) {
        ...
        const childEpics = epics.filter((e) => e.relationships.some((r) => r.target === milestone.id && r.type === "delivers"));
        for (const epic of childEpics) {
            console.log(`  ${formatStatus(epic.status)} ${epic.id}: ${epic.title}`);
            const childTasks = tasks.filter((t) => t.relationships.some((r) => r.target === epic.id && r.type === "delivers"));
    ...
    const orphanEpics = epics.filter((e) => !e.relationships.some((r) => r.type === "delivers" && milestones.some((m) => m.id === r.target)));
